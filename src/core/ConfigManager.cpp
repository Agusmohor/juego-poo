...
void ConfigManager::makeProfile() {
    std::ofstream file("data/config/profile.txt");
    file << "LastProfile = null" << std::endl;
    file.close();
}

void ConfigManager::saveConfig(const stats &p_stats, int vol, int &music, const std::array<sf::Keyboard::Scancode, 4> &keys) {
...
void ConfigManager::makeKeyBinds(std::array<sf::Keyboard::Scancode, 4> &keys) {
    {
        keybinds kb;
...
bool ConfigManager::loadProgess(const stats &p_stats, playerSaves &p_saves, std::vector<zombieSave> &z_saves, std::vector<treeSave> &t_saves) {
...
void ConfigManager::saveProgess(const stats &stats, playerSaves &p_saves, std::vector<zombieSave> &z_saves, std::vector<treeSave> &t_saves) {
...
void ConfigManager::newProgress(const stats &p_stats, playerSaves &p_saves, std::vector<zombieSave> &z_saves, std::vector<treeSave> &t_saves) {
        std::string path = "data/saves/" + p_stats.name + ".dat";
        std::ofstream file(path, std::ios::binary | std::ios::trunc);
        playerSaves s;
...
void ConfigManager::loadConfig(stats &p_stats, int &vol, int &music, std::array<sf::Keyboard::Scancode, 4> &keys) {
        {
